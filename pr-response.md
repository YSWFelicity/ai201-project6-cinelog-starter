...
I used an AI coding assistant throughout the review cycle, but treated its output as a starting point that had to be checked against the repository. For codebase orientation, I asked it to summarize `models.py` and `services/collection_service.py`, explain `add_to_collection()` step by step, and identify the structure used by `tests/test_collection.py`. I then verified those summaries directly in the files. That process helped me recognize the `verb_to_noun` naming convention, the service-layer duplicate query and domain-specific exception pattern, the model-level unique constraint, and the in-memory pytest fixture/application-context pattern. I used project-wide `rg` searches, isolated database exercises, and pytest runs to verify the actual implementation rather than relying on the summaries alone.

I also used AI to stress-test the arguments for Comments 4 and 5. For default visibility, I asked it to compare the sharing benefit of `public=True` with the risk of accidental disclosure. My final response keeps the public default to support CineLog's discovery behavior, but adds a condition that the UI must clearly disclose the default and provide accessible privacy controls; without those safeguards, I would choose private-by-default. For sorting, I asked it to evaluate alphabetical order against newest-first behavior. I accepted the maintainer's recency argument and strengthened it with consistency evidence from `get_collection()`, while preserving alphabetical sorting as a proposed future option rather than dismissing its value. Finally, I gave the cleaned `git log --oneline` output to AI for a Conventional Commits review, manually checked its concerns against each commit's diff and the official specification, and split an unrelated collection lookup change out of the initial watchlist feature commit.
